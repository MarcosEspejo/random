    ...
    <div className="min-h-screen bg-dark-300 flex items-center justify-center px-4 py-8 relative overflow-hidden">
      ...
      <div className="max-w-4xl w-full relative z-10">
        <div className="text-center space-y-6 md:space-y-12">
          ...
          {/* Features */}
          <div className="grid grid-cols-3 md:grid-cols-3 gap-4 md:gap-6 px-2">
            {features.map((feature, index) => (
              <div
                key={index}
                className="glass-effect rounded-lg p-4 md:p-6 text-center space-y-2 md:space-y-3 hover:border-gray-700 transition-colors duration-200"
              >
                ...
                <p className="text-gray-400 text-xs md:text-sm font-light hidden md:block">{feature.description}</p>
              </div>
            ))}
          </div>
          ...
          <div>
            <button
              type="button"
              onClick={handleStart}
              className="bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white text-lg md:text-lg font-semibold px-10 md:px-12 py-4 md:py-5 rounded-xl inline-flex items-center gap-3 shadow-md transition-colors duration-200 cursor-pointer"
            >
          ...
          {/* Info - Hidden on mobile to prevent scroll */}
